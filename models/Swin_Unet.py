...
class Swin_Unet(nn.Module):
    def __init__(self, img_size=512, patch_size=4, in_channels=3, num_classes=2,
    ...
    def forward(self, x):
        ...
        # =========================================在每个stage后应用CoordGate
        for i, gate in enumerate(self.coord_gates):
            if i < len(x_down_sample):
                # 将特征图从(B, L, C)转换为(B, C, H, W)格式
                h = w = int(math.sqrt(x_down_sample[i].shape[1]))
                feat = x_down_sample[i].transpose(1, 2).view(-1, self.embed_dim * 2 ** i, h, w)
                feat = gate(feat)
                # 转换回原始格式
                x_down_sample[i] = feat.flatten(2).transpose(1, 2)
